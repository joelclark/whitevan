...
#### Quotes
- **Quote = published estimate.** Setting `quote_status = Sent` makes the estimate available to the customer via a public approval link. `QuoteController::send` guards on `status === Ready` and all active line items having `unit_price` set. Send also emails the customer (if an email is on file) using `QuoteSentToCustomer` / `resources/views/emails/quotes/sent.blade.php`.
- **Customer-facing surface is the approval page.** `approval_token` (ULID, unique) is minted on first send and never changes. Public URL: `GET /approve/{approval_token}` — no auth, no signature, no expiry; the 128-bit token is the access control. No other customer-facing page exists; there is no read-only quote-review screen.
- **Approval controller.** `ApprovalFlowController::show` uses `Estimate::withoutGlobalScope('account')` (there's no authenticated user context). No view tracking: the controller does not stamp first-view timestamps, bump `last_activity_at`, or write activity / project-event rows. If tracking is reintroduced later, revisit `ApprovalFlowController` and the Projects `last_activity_at` bump list above.
- **Routes.** `routes/quotes.php`: public `GET /approve/{approval_token}`; authenticated `POST /estimates/{estimate}/send-quote`.
- **Frontend.** `resources/js/pages/quotes/approve.tsx` uses `quote-layout.tsx` (minimal public layout — no sidebar, no auth). Currently a placeholder with three "coming soon" steps (sign, deposit, confirm).
- **Activity events.** `EstimateQuoteSent` fires on send/re-send. No view event is recorded on the customer's end.
